/**
 * Custom Swing panel responsible for drawing the Monopoly board and player pieces.
 */
...
public class BoardPanel extends JPanel {
    ...
    private static final int CELL_PX = (BOARD_PX - 2 * CORNER_PX) / 9; // = 62
    ...
    /** Which edge the colour bar is on (inner board edge) */
    private String barSide(int pos) {
        if (pos >= 1  && pos <= 9)  return "top";    // bottom row → bar faces inward (top of cell)
        if (pos >= 11 && pos <= 19) return "right";  // left column → bar faces inward (right of cell)
        if (pos >= 21 && pos <= 29) return "bottom"; // top row → bar faces inward (bottom of cell)
        if (pos >= 31 && pos <= 39) return "left";   // right column→ bar faces inward (left of cell)
        return null;
    }
    ...
    private void drawTile(Graphics2D g, Tile tile) {
        ...
        // Text — rotate for left/right sides
        ...
        if (tile instanceof Property) {
            ...
            if (owner != null) {
                ...
                if (pi >= 0) {
                    g.setColor(PLAYER_COLORS[pi % PLAYER_COLORS.length]);
                    g.fillOval(r.x + r.width - 10, r.y + 2, 8, 8);
                    g.setColor(Color.WHITE);
                    g.setStroke(new BasicStroke(1f));
                    g.drawOval(r.x + r.width - 10, r.y + 2, 8, 8);
                }
            }
        }
    }
    ...
    private void drawCorner(Graphics2D g, Tile tile, Rectangle r) {
        ...
        if (tile instanceof GoTile) {
            ...
            fm = g.getFontMetrics();
            g.drawString("JAIL", cx - fm.stringWidth("JAIL") / 2, cy);

        } else if (tile instanceof FreeTile && tile.getPosition() == 10) {
            // pos 10 = Cascade Complex (Free Parking equivalent)
            g.setColor(BORDER_COL);
            ...
            g.setFont(new Font("Arial", Font.BOLD, 9));
            fm = g.getFontMetrics();
            g.drawString("Republic", cx - fm.stringWidth("Republic") / 2, cy - 3);
            g.drawString("Square", cx - fm.stringWidth("Square") / 2, cy + 9);
        }
    }
    ...
    private void drawTokens(Graphics2D g) {
        ...
        for (var entry : byPos.entrySet()) {
            ...
            int n = pis.size();
            // place tokens in bottom-right area
            for (int k = 0; k < n; k++) {
                int pi = pis.get(k);
